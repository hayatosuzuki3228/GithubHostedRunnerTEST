...
namespace Hutzper.Library.Forms.Setting
{
    ...
    /// ディレクトリ選択ユーザーコントロール
    /// </summary>
    public partial class DirectorySelectionUserControl : HutzperUserControl
    {
        /// <summary>
        /// テキスト
        /// </summary>
        public DirectoryInfo? Value
        {
            get
            {
                var value = (DirectoryInfo?)null;

                if (false == string.IsNullOrEmpty(this.watermarkTextbox1.Text))
                {
                    try
                    {
                        value = new DirectoryInfo(this.watermarkTextbox1.Text);
                    }
                    catch
                    {
                    }
            ...
            set
            {
                ...
                this.watermarkTextbox1.Text = text;
                this.watermarkTextbox1.SelectionStart = text.Length;
        ...
        /// </summary>
        [Category("Function")]
        public bool DirectorySelectable { get => this.noFocusedButton1.Visible; set => this.noFocusedButton1.Visible = value; }
        ...
        {
            get => this.watermarkTextbox1.Font;
            set => this.watermarkTextbox1.Font = value;
        }
        ...
        /// コンストラクタ
        /// </summary>
        public DirectorySelectionUserControl()
        {
            this.InitializeComponent();

            this.Size = new Size(460, 46);
        ...
        private void DirectorySelectionUserControl_Resize(object sender, EventArgs e)
        {
            try
            {
                // ラベル更新
        ...
        private void noFocusedButton1_Click(object sender, EventArgs e)
        {
            try
            {
                if (DialogResult.OK == this.folderBrowserDialog.ShowDialog(this))
                {
                    this.watermarkTextbox1.Text = this.folderBrowserDialog.SelectedPath;
                }
            }
            catch (Exception ex)
            {
        ...
        private void watermarkTextbox1_DoubleClick(object sender, EventArgs e)
        {
            if (false == string.IsNullOrEmpty(this.watermarkTextbox1.Text))
            {
                this.watermarkTextbox1.SelectAll();
                Clipboard.SetText(this.watermarkTextbox1.Text);
            }
        }
    }
}