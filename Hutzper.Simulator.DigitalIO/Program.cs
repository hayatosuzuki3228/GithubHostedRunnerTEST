...
namespace Hutzper.Simulator.DigitalIO
{
    internal static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // 起動制御
            SelfBootController? selfBootController = null;
            ...
            try
            {
                ...
                {
                    // Form
                    ServiceCollectionSharing.Instance.ServiceCollection.AddSingleton<Form1>();
                ...
                var logPath = System.IO.Path.Combine(appConfigFile.Directory?.Parent?.FullName ?? "..\\", "log_SImulator.DigitalIO");

                // ログの設定
                // ここにSerilogの初期化書く
                UseSerilog.StartLogger(logType: LogType.DigitalIO);
                ...
                // 起動フォーム生成
                var form = ServiceCollectionSharing.Instance.ServiceProvider?.GetRequiredService<Form1>();
