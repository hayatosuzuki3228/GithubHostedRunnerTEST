...
namespace Hutzper.Library.Common.Net.Sockets
{
    ...
    /// </summary>
    [Serializable]
    public class TcpBinaryCommunicationListener<T> : SafelyDisposable, ILoggable where T : ITcpBinaryCommunicationLinkage, new()
    {
        #region ILoggable
        ...
        public delegate void AcceptTcpClientEventHandler(object sender, TcpBinaryCommunicationClient<T> client, out bool notUseClient);
        ...
        public TcpBinaryCommunicationListener()
        {
        }
                ...
                void listeningProcess(TcpListener activeListener, ThreadControlFlag controlFlag)
                {
                    try
                    {
                        ...
                        while (false == controlFlag.IsTerminate)
                        {
                            ...
                            // 通信クライアント
                            var comClient = new TcpBinaryCommunicationClient<T>();

                            #region 通信クライアントに接続を割り当てイベント通知する
                            try
                            {
                                // クライアント接続割り当て
                                comClient.Attach(acceptClient);
                                ...
                                try
                                {
                                    ...
                                    this.AcceptTcpClient?.Invoke(this, comClient, out notUseClient);
                                    if (true == notUseClient)
                                    {
                                        // 通信クライアント破棄
                                        this.DisposeSafely(comClient);
                                        comClient = null;
