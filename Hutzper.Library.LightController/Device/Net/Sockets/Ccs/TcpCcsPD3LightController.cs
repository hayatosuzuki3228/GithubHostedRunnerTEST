...
namespace Hutzper.Library.LightController.Device.Net.Sockets.Ccs
{
    ...
    /// CCS TCP通信照明制御（PD3）
    /// </summary>
    public class TcpCcsPD3LightController : LightControllerBase, ITcpLightController
    {
                            ...
                            if (true == this.CommunicationClient.Connect(this.CommunicationParameter.IpAddress, this.CommunicationParameter.PortNumber))
                            {
                                if (this.CommunicationParameter is ILightControllerParameter)
                                {
                                    ...
                                    #region IPアドレス変更
                                    {
                                        //Task.Run(async () =>
                                        //{
                                        //    var command = this.FormatCommand($"{0:D2}E01{"192.168.127.100"}");

                                        //    var result = await this.Handshake(command);

                                        //    this.VerifyCommand(result, out string value);

                                        //    command = this.FormatCommand($"{0:D2}E05{"192.168.127.101"}");
        ...
        public override bool Modulate(int channel, double value)
        {
            ...
            try
            {
                ...
                Task.Run(async () =>
                {
                    var command = this.FormatCommand($"{this.GetChannelText(channel)}F{valueInt:D3}");
                    var result = await this.Handshake(command);
                    isSuccess = this.VerifyCommand(result, out string value);
                }).ConfigureAwait(false).GetAwaiter().GetResult();
            }
                ...
                var result = Task<double>.Run(async () =>
                {
                    ...
                    try
                    {
                        ...
                        if (true == this.VerifyCommand(text, out string valueText))
                        {
                            if (valueText[0] == 'F')
                            {
                                value = double.Parse(valueText.Substring(1, 3));
                            }
                        }
                    }
                    catch (Exception ex)
        ...
        public override bool ChangeExternalTriggerStrobeTimeUs(int channel, int strobeTimeUs = 0)
        {
            ...
            try
            {
                ...
                if (0 < strobeTimeUs)
                {
                    ...
                    valueInt = selection.IndexOf(selection.Min()) + 1;
                }

                Task.Run(async () =>
                {
                    var command = this.FormatCommand($"{this.GetChannelText(channel)}S{valueInt:D2}");
        ...
        public override Task<int> ExternalTriggerStrobeTimeUsAsync
        {
            get
            {
                var result = Task<int>.Run(async () =>
                {
                    ...
                    try
                    {
                        var command = this.FormatCommand($"{this.GetChannelText(this.Channel):D2}M");
                        ...
                        if (true == this.VerifyCommand(text, out string valueText))
                        {
                            if (valueText[5] == 'S')
                            {
                                var valueInt = int.Parse(valueText.Substring(6, 2));
                                ...
                                if (0 < valueInt && choiceValues.Count < valueInt)
                                {
                                    value = choiceValues[valueInt];
                                }
                            }
                        }
                    }
        ...
        /// コンストラクタ
        /// </summary>
        public TcpCcsPD3LightController() : this(typeof(TcpCcsPD3LightController).Name, Common.Drawing.Point.New())
        {
        ...
        /// コンストラクタ
        /// </summary>
        public TcpCcsPD3LightController(Common.Drawing.Point location) : this(typeof(TcpCcsPD3LightController).Name, location)
        {
        ...
        public TcpCcsPD3LightController(string nickname, Common.Drawing.Point location) : base(nickname, location)
        {
        ...
        protected virtual bool VerifyCommand(string command, out string value)
        {
            ...
            if (false == string.IsNullOrEmpty(command) && command.Length >= 8)
            {
                var body = command[1..^4];
